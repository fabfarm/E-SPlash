...
// Define board-specific configuration settings from the chosen board configuration
...
void setup(){
  ...
  scanWifi();
  startWifi();
  disableAllRelays();  // Set all relays off at startup
  printCompileTime();  // Prints compile time to serial monitor
  setupRTC();  // Setup Real Time Clock
  setupStorage();  // Setup storage
  server.begin();  // Start the HTTP server
  // Routes to data endpoints for the web app
...
// Function to start setup serial monitor
void setupSerialMonitor() {
  Serial.begin(9600);
}
...
void readDataJson() {
  ...
  // Open the data.json file or sample.json file if data.json does not exist
  File f;
  ...
  // Deserialize the JSON data and store it in the global variable 'doc'
...
/*
 * This function writes the settings in the 'doc' JSON object to the data.json file on the SPIFFS file system
 */
bool writeDataJson() {
  String jsonString;
  
  // Serialize the JSON data from the 'doc' object and store it in 'jsonString'
  Serial.println("JSON serialize - BEGIN");
  serializeJson(doc, jsonString);
  Serial.println("JSON serialize - COMPLETE");

  // Write 'jsonString' to the data.json file on the SPIFFS file system
...
void scheduleMode() {
  ...
  // Access the "relays" array from the "doc" JSON object
  ...
  for (int i = 0; i < relays.size(); i++){
    ...
    // Get the GPIO pin number for the relay
    int pinNumber = relays[i]["pin"];

    // Loop through each time slot for the current relay
...
/*This function turn pump on and off in case manual mode is selected*/
void manualMode()
{
    // Print a message to indicate that manual mode has been activated
    Serial.println("manualMode");

    // Initialize a boolean variable to keep track of whether any relays are on
    int enablePump = false;

    // Get the "relays" JSON array from the "doc" object
    JsonArray relays = doc["relays"];

    // Loop through each relay in the "relays" array
    for (int i = 0; i < relays.size(); i++)
    {
        // Get the GPIO pin number for the current relay
        int pinNumber = relays[i]["pin"];

        // Set the GPIO pin to output mode
        pinMode(pinNumber, OUTPUT);

        // If the current relay is enabled
        if (relays[i]["isEnabled"])
        {
            // Set the GPIO pin to the "on" state
            digitalWrite(pinNumber, ON);

            // Print a message to indicate that the relay is on
            Serial.printf("Relay %s (pin %d) is on\n\r", relays[i]["name"].as<char*>(), pinNumber);

            // Set the "enablePump" variable to true since at least one relay is on
            enablePump = true;
        }
        else
        {
            // Set the GPIO pin to the "off" state
            digitalWrite(pinNumber, OFF);

            // Print a message to indicate that the relay is off
            Serial.printf("Relay %s (pin %d) is off\n\r", relays[i]["name"].as<char*>(), pinNumber);
        }
    }

    // Turn off the pump if no relays are on
    switchPump(enablePump);
}


/*
 * Function to deactivate all relays. This is to ensure a safe startup.
 */
void disableAllRelays()
{
  // Disable pump relay
  ...
  // Loop over configured relays and turn them off
  JsonArray relays = doc["relays"];
  for (int i = 0; i < relays.size(); i++)
  {
    // Get GPIO pin number for the current relay
    int pinNumber = relays[i]["pin"];
    ...
    // Print a message to the Serial monitor indicating that the relay has been turned off
...
bool isWithinTimeslot (int startTimeInMinutes, int duration)
{
  if (duration <= 0)
  {
    // If duration is zero or negative, return false immediately
    return false;
  }
...
float readDHTTemperature() {
  ...
  const unsigned long readInterval = 2000; // Reading interval in ms
  ...
  // Read temperature only if enough time has passed
  ...
  // Return NAN if not enough time has passed since the last read
  return NAN;
}
 ...
 */
void assignLocalTime(){
  // Declare variables for time components
  int THour; 
  int TMin; 
  int TSec; 
  int TYear;
  ...
  // Define the NTP server, GMT offset, and daylight offset
  const char* ntpServer = "pool.ntp.org";
  const long gmtOffset_sec = 3600; // GMT+1
  const int daylightOffset_sec = 0;

  // Configure time using the NTP server, GMT offset, and daylight offset
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);

  // Get the current local time and store it in the timeinfo struct
  struct tm timeinfo;
  getLocalTime(&timeinfo);

  // Extract the time components from the timeinfo struct
  ...
  // Set the RTC time using the time components
  rtc.setTime(TSec, TMin, THour, TDay, TMonth, TYear);
}

// Define a macro to calculate the length of an array
#define countof(a) (sizeof(a) / sizeof(a[0]))

// Print the date and time stored in an RtcDateTime object
void printDateTime(const RtcDateTime& dt)
{
  // Create a string to hold the formatted date and time
  char datestring[20];

  // Format the date and time string using the provided format
...
void setupRTC() {
  ...
  // Set RTC time to the compilation time of the program
  RtcDateTime compiled = RtcDateTime(__DATE__, __TIME__);

  // Print the time before updating from external RTC
  Serial.println("* ");
  ...
  // Print the time after updating from external RTC
  Serial.println("* ");
  ...
  if (!Rtc.IsDateTimeValid()) {
    ...
    Rtc.SetDateTime(compiled); // Set RTC time to the compilation time of the program
  }

  // Enable writing to RTC if write protection is enabled
  #ifdef ds_3231
  #else
  ...
  // Check if RTC time is older or newer than compilation time
...
/*
 * This function prints the current time on the serial monitor at a predetermined interval using RTC
 */
void testRtcOnLoop()
{
  // Declare and initialize the variables for the interval
  unsigned long previousMillis = 0;
  const long printTimeInterval = 1000; // 1 second

  Serial.println("This data comes from void testRtcOnLoop(). Time will appear every 1 second.");

  // Get the current time and check if the print interval has elapsed
  ...
  if (currentMillis - previousMillis >= printTimeInterval) 
  {
    ...
    // Get the current date and time from the RTC and print it to the serial monitor
    RtcDateTime now = Rtc.GetDateTime();
    if (now.IsValid())
    {
    ...
    {
      // Print an error message if the RTC lost confidence in the date and time
      Serial.println("RTC lost confidence in the DateTime!");
    }
  }
...
/*
 * Function with WiFi settings and initialization
 */
void setupWifi()
{
  ...
  // Start a soft AP with the given credentials and maximum connections
  WiFi.softAP(soft_ap_ssid, soft_ap_password, 3);

  // Print the IP address of the soft AP to the serial monitor
  Serial.println();
  Serial.println("*****************************************************");
  Serial.printf("* SoftAP IP is: %s\n\r", WiFi.softAPIP().toString().c_str());

  #ifdef stacticIP
  // If a static IP is defined, configure the STA with the static IP settings
  if (!WiFi.config(local_IP, gateway, subnet, primaryDNS, secondaryDNS)) {
    Serial.println("* STA Failed to configure");
  }
  #endif

  // Set the hostname of the STA to the defined hostname
  WiFi.setHostname(wifi_network_hostname);

  // Print a message to the serial monitor indicating the device is waiting for a WiFi network
 ...
 */
void changeTimeFromJSON() {
  // Extract the necessary fields from the JSON document
